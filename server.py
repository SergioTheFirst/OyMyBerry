...
# -*- coding: utf-8 -*-
"""
OyMyBerry - Local Development Server
...
def generate_sitemap():
    ...
    site_url = config.get('site', {}).get('url', 'https://oymyberry.ru')
    today = get_date()
...
class OyMyBerryHandler(http.server.SimpleHTTPRequestHandler):
    """Custom HTTP request handler for OyMyBerry."""
    ...
    def do_GET(self):
        ...
        if path.endswith(('.py', '.pyc', '.pyo', '.bat')) or filename == 'secrets.json':
    ...
    def do_POST(self):
        ...
        # API only from localhost — block external access
        ...
        elif path == '/update-work':
            self.handle_update_work()
    ...
    def handle_upload(self):
        ...
        try:
            ...
            category = parts.get('category', [b'other'])[0].decode('utf-8').strip()
    ...
    def handle_update_work(self):
        """Handle work update — edit title, description, category."""
        try:
            content_length = int(self.headers.get('Content-Length', 0))
            body = self.rfile.read(content_length)
            data = json.loads(body.decode('utf-8'))
            ...
            write_json(WORKS_JSON, works_data)
            self.log_message('Updated work: %s', title)
            self.send_json({'success': True, 'work': found})

        except Exception as e:
            self.log_message('Update error: %s', str(e))
            self.send_json({'error': str(e)}, 500)
    ...
    def handle_git_push(self):
        ...
        try:
            ...
            def github_api(method, url, data=None):
                ...
                req.add_header('User-Agent', 'OyMyBerry-Server')
                if req_body:
...
def main():
    ...
    server = http.server.HTTPServer((HOST, PORT), OyMyBerryHandler)
    url = 'http://localhost:%d' % PORT

    print('', flush=True)
    print('  ====================================', flush=True)
    print('       OyMyBerry Server', flush=True)
    print('  ====================================', flush=True)
    print('', flush=True)
